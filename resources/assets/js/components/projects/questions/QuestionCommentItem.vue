<template>
    <div class="list-group-item" :class="{ 'bg-light': question.resolved }">
      <div class="d-flex flex-column">
        ...
        <span style="font-size:.75rem;">{{comment.created_at | datetime}} <span v-if="ownsComment">• <a href="#" class="text-muted" @click.prevent="deleteComment(comment)">Delete</a></span></span>
      </div>
    </div>
</template>
<script>
